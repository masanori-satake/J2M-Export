...
- `--overwrite`: 同名ファイルが存在する場合に上書きする（指定しない場合はSuffixが付与されます）
- `--no-resume`: 途中からの再開を行わず、最初から新規エクスポートを実行します
...
- **自動進捗保存と再開**:
  大量のチケット取得中に 504 Gateway Timeout や一時的なネットワーク障害が発生した場合、それまで処理したデータを即座にファイル出力し、出力ディレクトリに進捗状態ファイル（`.j2m_resume.json`）を保存します。
- **再実行時の動作**:
  障害解消後に同一の検索条件で再度コマンドを実行すると、ステートファイルから処理済みチケット一覧を自動読み込みし、中断した箇所からエクスポートを再開します。非上書きモードの場合も、前回生成されたファイル名（タイムスタンプ）を引き継ぎます。追記するのはステートファイルに記録された番号の出力ファイルだけです。そのファイルがサイズ上限に達した場合は次の番号から分割保存し、同名の古い分割ファイルがあれば書き直します。
- **クリーンアップと新規実行**:
  全チケットのエクスポートが正常に完了すると、ステートファイルは自動削除されます。途中で最初からやり直したい場合は `--no-resume` オプションを指定して実行するか、`.j2m_resume.json` を削除してください。
...
- **504 Gateway Timeout / 最大リトライ回数超過による中断**:
  - 現象: 取得の途中で504エラー等のネットワーク例外が発生し停止する。
  - 原因: レート制限やJiraサーバーの一時的な高負荷。
  - 対処: それまでに取得できたデータはファイルに保存され、`.j2m_resume.json` に進捗が記録されています。ネットワーク状態やJiraの状態を確認後、再度コマンドを実行することで途中からエクスポートを再開できます。
- **Configuration error**:
  - 現象: 実行時にエラーが発生して停止する。
